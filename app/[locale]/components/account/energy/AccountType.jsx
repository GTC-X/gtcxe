...
export default function AccountTypeEnergy() {
  const [currentTab, setCurrentTab] = useState(0);
  const locale = useLocale();
  const t = useTranslations("Instruments");
  const tabs = [
    {
      label: t("energy.tab1.tabName"),
    },
    {
      label: t("energy.tab2.tabName"),
    },
    {
      label: t("energy.tab3.tabName"),
    },
    {
      label: t("energy.tab4.tabName"),
    },
    {
      label: t("energy.tab5.tabName"),
    },
  ];
  ...
  const getData = (index) => {
    switch (locale) {
      case "en":
        return enAccountTableData?.energyData?.[index];
      case "ar":
        return arAccountTableData?.energyData?.[index];
      case "zh":
        return zhAccountTableData?.energyData?.[index];
      default:
        return [];
    }
  };
          ...
          <div className="tab-content ">
            <div className="reason-content py-1">
              <h2 className="font-semibold text-2xl text-primary pb-3">
                {currentTab == 4
                  ? t("energy.tab4.subHeading")
                  : t("energy.tab1.subHeading")}
              </h2>
            </div>
          </div>
          <Tab.Panels>
