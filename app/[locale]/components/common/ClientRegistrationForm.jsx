...
const ClientRegistrationForm = (props) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      // Check if email OTP is verified before submitting (for non-EUS countries)
      if (countryCode != "EUS" && showEmailOtpVerify && !isEmailVerified) {
        toast.error(t("otp_not_verified") || "Please verify email OTP before submitting");
        return;
      }

      // Check if phone OTP is verified before submitting (for EUS countries)
      if (countryCode == "EUS" && showPhoneOtpVerify && !isPhoneVerified) {
        toast.error(t("otp_not_verified") || "Please verify phone OTP before submitting");
        return;
      }
